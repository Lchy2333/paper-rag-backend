...
func (f Font) getEncoder() TextEncoding {
	...
	case Name:
		...
		case "GBK-EUC-H", "GBK-EUC-V", "GBK2K-H", "GBK2K-V":
			return &gbkEncoder{}
		default:
			if DebugOn {
...
func (e *gbkEncoder) Decode(raw string) (text string) {
	out, _, err := transform.Bytes(simplifiedchinese.GBK.NewDecoder(), []byte(raw))
	if err != nil {
		return raw
	}
	return string(out)
...
func (p Page) Content() Content {
	...
	showText := func(s string) {
		...
		for _, ch := range decoded {
			...
			// Width() returns 0 for many CID / simple fonts that have no
			// /Widths array on the font dictionary (base-14 Times-Roman,
			// subset CJK fonts, etc.). A zero advance makes every glyph of a
			// run share the same X, so the caller's (Y, X) sort scrambles the
			// reading order. Fall back to a font-size based advance so X
			// stays monotonic in reading order.
			if w0 == 0 {
				if ch < 0x1100 {
					w0 = g.Tfs * 0.5 // half-width (ASCII, Latin, digits, space)
				} else {
					w0 = g.Tfs // full-width (CJK)
				}
			}
	...
	var process func(strm Value, res Value)
	var paintXObject func(args []Value, res Value)

	// paintXObject 处理 Do 运算符：若引用的是 Form XObject，则应用其 Matrix，
	// 用其（或继承的）资源递归处理其内容流。多个表单共享同一 g/text/rect 状态。
	paintXObject = func(args []Value, res Value) {
		if len(args) != 1 {
			return
		}
		xobj := res.Key("XObject").Key(args[0].Name())
		if xobj.IsNull() || xobj.Key("Subtype").Name() != "Form" {
			return
		}
		savedG := g
		savedEnc := enc
		...
		process(xobj, formRes)
		g = savedG
		enc = savedEnc
	}

	process = func(strm Value, res Value) {
		Interpret(strm, func(stk *Stack, op string) {
			n := stk.Len()
			args := make([]Value, n)
			for i := n - 1; i >= 0; i-- {
			...
			case "Do": // paint Form XObject (recursively process its content)
				paintXObject(args, res)
	...
	process(p.V.Key("Contents"), p.Resources())
